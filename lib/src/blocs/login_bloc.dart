...
class LoginBloc extends Bloc<LoginEvent, LoginState> with ValidationMixin {
  ...
  Stream<String> get password => _passwordController.stream.transform(passwordValidate);
  Stream<bool> get submitValid => Observable.combineLatest2(email, password, (e, p) {
    return true;
  });
  ...
  Stream<LoginState> mapEventToState(LoginEvent event) async* {
    if (event is PressLogin) {
      ...
      try {
        ...
        final loginResponse = await userRepository
            .authenticate("eve.holt@reqres.in", "cityslicka");
